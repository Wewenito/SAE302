...
class Server_handle:
    ...
    def conn_buffer(self, connection, exit_flag):
        #This function is here in case multiple people connect at the same time, we divide conn inits in multiple threads to not block other people
        action = connection[1].recv(1024).decode()#this part can block the /kill function until user has clicked the login / register button

        if action == "":
            print("Client got disconnected, clearing cache.")
    ...
    def listen_for_conns(self, exit_flag):
        ...
        while not exit_flag.is_set():
            try:
                ...
                buffer_thread = threading.Thread(target=self.conn_buffer, args=(connection, exit_flag))
                buffer_thread.start()
    ...
    def ban_user(self, user_to_ban):
        ...
        if connection is not None:#if user is currently connected
            for client in self.current_threads:
                if client[0] == connection:
                    ip_adddr, port = client[1].getpeername()
                    break
                    ...
                    if does_user_exists(mydb, user_data_proposed[0]):
                        ...
                        if user_data_from_db['is_banned'] == 1:
                            ...
                            if kick_or_ban[0] == True:
                                if kick_or_ban[1] == "KICK":
                                    ...
                                    conn_ip = client_conn[1].getpeername()
                        ...
                        if user_data_proposed[1] == user_data_from_db['Password']:
                            ...
                            for friends in user_friends:
                                ...
                                packet = packet + f"{friends[2]}"+ "§" + f"{friend_info['Username']}" + "§" + f"{friends[3]}" + "/"
        ...
        try:
            ...
            while client_conn[2] == False and not exit_flag.is_set():
                try:
                    ...
                    current_ip = client_conn[1].getpeername()
        ...
        while not exit_flag.is_set():
            try:
                ...
                if message == "":
                    ...
                    elif message_processed[0] == "FRIEND_REQUEST":
                        demand_from = message_processed[1]#user_id
                        demand_to = message_processed[2]#user_name
                        user_type = message_processed[3]#'FORCE' if admin, 'NORMAL' if normal user
                
                        mydb = establish_connection()
                        user_to_add = get_user_info(mydb, demand_to)

                        if user_to_add is not None:
                            action = set_friend_request(mydb, demand_from, user_to_add['id'], user_type)
                            if action[0] == True:
                                if user_type == "FORCE":
                                    response = f"FRIEND_REQUEST|SUCCESS_ADMIN|{user_to_add['id']}|{demand_to}"
                                else:
                                    response = "FRIEND_REQUEST|SUCCESS"
                                connection[1].send(response.encode())
                            else:
                                if action[1] == "USERS_ARE_ALREADY_FRIENDS":
                                    response = "FRIEND_REQUEST|USERS_ARE_ALREADY_FRIENDS"
                                    connection[1].send(response.encode())
                                elif action[1] == "USER_ALREADY_SENT_REQUEST":
                                    response = "FRIEND_REQUEST|USER_ALREADY_SENT_REQUEST"
                                    connection[1].send(response.encode())
                                elif action[1] == "USER_WANTS_TO_BE_FRIEND_ALREADY":
                                    response = "FRIEND_REQUEST|USER_WANTS_TO_BE_FRIEND_ALREADY"
                                    connection[1].send(response.encode())
                                else:
                                    print(f"Error during function {action[1]}")
                        else:
                            response = "FRIEND_REQUEST|USER_DOES_NOT_EXISTS"
                            connection[1].send(response.encode())
                    elif message_processed[0] == "ACCEPT_FRIEND_REQUEST":
                        demand_from = message_processed[1]
                        demand_to = message_processed[2]
                        decision = message_processed[3]

                        mydb = establish_connection()
                        result = update_friend_request(mydb, demand_from, demand_to, decision)

                        if result[0] == True:
                            if result[1] == "UPATE_OK_USERS_ARE_FRIENDS":
                                connection[1].send(f"UPDATE_FRIEND|UPATE_OK_USERS_ARE_FRIENDS|{demand_from}".encode())
                            
                                for client in self.user_data:
                                    if client[1]['id'] == int(demand_from):
                                        for conn in self.current_threads:
                                            if conn[0] == client[0]:
                                                print("User is currently connected, sending him the update")
                                                # print(f"USER DATA IN QUESTION : {client}")
                                                # print(f"CONN IN QUESTION : {conn}")
                                                conn[1].send(f"UPDATE_FRIEND|NEW_FRIEND_AVAILABLE|{demand_to}".encode())
                                                break
                            elif result[1] == "UPATE_OK_USERS_ARENT_FRIENDS":
                                connection[1].send(f"UPDATE_FRIEND|UPATE_OK_USERS_ARENT_FRIENDS|{demand_from}".encode())
                            else:
                                print("something unexpected occured")
                        else:
                            print("Error while updating friend request")
                            print(result[1])
                    else:
                        pass
            except socket.timeout:
            ...
            if first_space_index != -1: #If command is a command keyword + string (ex : /join Salon1 or /broadcast this is a global broadcast)
                ...
                match split[0]:
                        ...
                        case "/accept_friend_request":
                            demand_from = split[1].split("|")[0]
                            demand_to = split[1].split("|")[1]
                            judgement = split[1].split("|")[2]
                            return ["ACCEPT_FRIEND_REQUEST", demand_from, demand_to, judgement]
                        case "/friend_request":
                            demand_from = split[1].split("|")[0]
                            demand_to = split[1].split("|")[1]
                            user_type = split[1].split("|")[2]
                            return ["FRIEND_REQUEST", demand_from, demand_to, user_type]
                        case _:
                            return ["", split[1]]
